...
contract STVRMFactory is ModuleFactory {
    ...
    */
    function deploy(bytes _data) external returns(address) {
        ...
        STVRManager stvrManager = new STVRManager(msg.sender, address(polyToken));

        require(Util.getSig(_data) == stvrManager.getInitFunction(), "Provided data is not valid");
        require(address(stvrManager).call(_data), "Un-successfull call");
        emit GenerateModuleFromFactory(address(stvrManager), getName(), address(this), msg.sender, setupCost, now);
        return address(stvrManager);
    }
