...
contract('WeightedVoteCheckpoint', accounts => {
    ...
    describe("Create ballot" , async() => {
        ...
        it("Should fail to create a new ballot if not owner" async() => {
            let errorThrown = false;
            try {
                let tx = await I_WeightedVoteCheckpoint.createBallot(duration.minutes(2), { from: account_temp });
            } catch(error) {
        ...
        it("Should successfully create a new ballot" async() => {
            let tx = await I_WeightedVoteCheckpoint.createBallot(duration.minutes(2), { from: token_owner });
    ...
    describe("Create custom ballot", async() => {
        ...
        it("Should fail to create a new custom ballot with endTime before startTime" async() => {
            let errorThrown = false;
            try {
                let startTime = latestTime() + duration.minutes(2);
                let tx = await I_WeightedVoteCheckpoint.createCustomBallot(startTime, latestTime(), 1, { from: token_owner });
            } catch(error) {
        ...
        it("Should fail to create a new custom ballot if checkpointId does not exist" async() => {
            ...
            try {
                ...
                let tx = await I_WeightedVoteCheckpoint.createCustomBallot(latestTime(),endTime, 10, { from: token_owner });
            } catch(error) {
        ...
        it("Should fail to create a new custom ballot if not owner" async() => {
            ...
            try {
                let endTime = latestTime() + duration.minutes(2);
                let tx = await I_WeightedVoteCheckpoint.createCustomBallot(latestTime(),endTime, 1, { from: account_temp });
            } catch(error) {
                console.log(`       tx -> failed because msg.sender is not owner`.grey);
                ensureException(error);
                errorThrown = true;
    ...
    describe("Cast vote", async() => {
        ...
        it("Should fail to cast a vote before voting period has started" async() => {
            let startTime = latestTime() + duration.minutes(2)
            let endTime = latestTime() + duration.minutes(2);
            await I_WeightedVoteCheckpoint.createCustomBallot(startTime,endTime, 1, { from: token_owner });

            
        });

        it("Should fail to cast a vote after voting period has ended" async() => {
