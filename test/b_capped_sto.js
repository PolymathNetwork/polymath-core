...
contract('CappedSTO', accounts => {
  ...
  describe('verify the data of STO', async () => {
    it('Should verify the configuration of the STO', async () => {
      ...
      assert.equal(await I_CappedSTO_Array_ETH[0].fundRaiseTypes.call(E_fundRaiseType), true, "STO Configuration doesn't set as expected");
    });
  });
  ...
  describe('Buy tokens', async () => {
    ...
    it('Should restrict to buy tokens after hiting the cap in second tx first tx pass', async () => {
      let tx = await I_GeneralTransferManager.modifyWhitelist(account_investor2, fromTime, toTime + duration.days(20), expiryTime, true, {
        from: account_issuer,
        gas: 500000
      });
  ...
  describe('Attach second ETH STO module', async () => {
    ...
    it('Should verify the configuration of the STO', async () => {
      ...
      assert.equal(await I_CappedSTO_Array_ETH[1].fundRaiseTypes.call(E_fundRaiseType), true, "STO Configuration doesn't set as expected");
    });
  ...
  describe('Test cases for reaching limit number of STO modules', async () => {
    ...
    it('Should successfully invest in all STO modules attached', async () => {
      ...
      for (var STOIndex = 2; STOIndex < MAX_MODULES; STOIndex++) {
        ...
        assert.equal((await I_CappedSTO_Array_ETH[STOIndex].getRaised.call(ETH)).dividedBy(new BigNumber(10).pow(18)).toNumber(), 1);
  ...
  describe('Test Cases for an STO of fundraise type POLY', async () => {
    ...
    describe('Buy tokens', async () => {
      it('Should Buy the tokens', async () => {
        ...
        assert.equal((await I_SecurityToken_POLY.balanceOf(account_investor1)).dividedBy(new BigNumber(10).pow(18)).toNumber(), 5000);
      });
      ...
      it('Should restrict to buy tokens after hiting the cap in second tx first tx pass', async () => {
        ...
        assert.equal((await I_SecurityToken_POLY.balanceOf(account_investor2)).dividedBy(new BigNumber(10).pow(18)).toNumber(), 45000);
  ...
  describe('Attach second POLY STO module', async () => {
    ...
    it('Should verify the configuration of the STO', async () => {
      ...
      assert.equal((await I_CappedSTO_Array_POLY[1].endTime.call()).toNumber(), endTime_POLY2, "STO Configuration doesn't set as expected");
      assert.equal(
      ...
      assert.equal(await I_CappedSTO_Array_POLY[1].fundRaiseTypes.call(P_fundRaiseType), true, "STO Configuration doesn't set as expected");
    });
    ...
    it('Should successfully invest in second STO', async () => {
      ...
      assert.equal((await I_CappedSTO_Array_POLY[1].getRaised.call(POLY)).dividedBy(new BigNumber(10).pow(18)).toNumber(), polyToInvest);

      assert.equal(await I_CappedSTO_Array_POLY[1].investorCount.call(), 1);

      assert.equal((await I_SecurityToken_POLY.balanceOf(account_investor3)).dividedBy(new BigNumber(10).pow(18)).toNumber(), stToReceive);
    });
  });
});
