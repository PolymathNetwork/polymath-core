...
contract("SecurityTokenRegistry", async (accounts) => {
    ...
    describe("Generate SecurityToken", async () => {
        ...
        it("Should fail to generate the securityToken -- Because ticker length is 0", async () => {
            ...
            await catchRevert(
                I_STRProxied.generateSecurityToken(name, "0x0", tokenDetails, false, address_zero, { from: token_owner }),
        ...
        it("Should fail to generate the securityToken -- Because name length is 0", async () => {
            await catchRevert(
                I_STRProxied.generateSecurityToken("", symbol, tokenDetails, false, address_zero, { from: token_owner }),
        ...
        it("Should fail to generate the securityToken -- Because msg.sender is not the rightful owner of the ticker", async () => {
            await catchRevert(
                I_STRProxied.generateSecurityToken(name, symbol, tokenDetails, false, address_zero, { from: account_temp }),
        ...
        it("Should generate the new security token with the same symbol as registered above", async () => {
            ...
            assert.equal(await I_SecurityToken.getTreasuryWallet.call(), treasury_wallet, "Incorrect wallet set")
        ...
        it("Should get all created security tokens", async() => {
            ...
            let tx = await I_STRProxied.generateSecurityToken(name, "TMP", tokenDetails, false, address_zero, { from: account_temp });
    ...
    describe("Generate SecurityToken v2", async () => {
        ...
        it("Should generate the new security token with version 2", async () => {
            ...
            let tx = await I_STRProxied.generateSecurityToken(name2, symbol2, tokenDetails, false, address_zero, { from: token_owner });
    ...
    describe("Test cases for the changeTickerRegistrationFee()", async () => {
        ...
        it("Should fail to launch the securityToken with the old launch fee", async () => {
            await I_PolyToken.approve(I_STRProxied.address, initRegFeePOLY, { from: token_owner });
            await catchRevert(
                I_STRProxied.generateSecurityToken("Polymath", "POLY", tokenDetails, false, address_zero, { from: token_owner }),
        ...
        it("Should launch the the securityToken", async () => {
            await I_PolyToken.approve(I_STRProxied.address, new BN(web3.utils.toWei("2000")), { from: token_owner });
            let tx = await I_STRProxied.generateSecurityToken("Polymath", "POLY", tokenDetails, false, address_zero, { from: token_owner });
