...
import { setUpPolymathNetwork, deployLockupVolumeTMAndVerified } from "./helpers/createInstances";
import { catchRevert } from "./helpers/exceptions";

const SecurityToken = artifacts.require('./SecurityToken.sol');
const GeneralTransferManager = artifacts.require('./GeneralTransferManager');
const LockupVolumeTransferManager = artifacts.require('./LockupVolumeTransferManager');
...
contract('LockupVolumeTransferManager', accounts => {
    ...
    let P_LockupVolumeTransferManagerFactory;
    let I_SecurityTokenRegistryProxy;
    let P_LockupVolumeTransferManager;
    let I_GeneralTransferManagerFactory;
    let I_LockupVolumeTransferManagerFactory;
    let I_GeneralPermissionManager;
    let I_LockupVolumeTransferManager;
    ...
    before(async() => {
        ...
        // STEP 4(c): Deploy the LockupVolumeTransferManager
        [I_LockupVolumeTransferManagerFactory] = await deployLockupVolumeTMAndVerified(account_polymath, I_MRProxied, I_PolyToken.address, 0);
        // STEP 4(d): Deploy the LockupVolumeTransferManager
        [P_LockupVolumeTransferManagerFactory] = await deployLockupVolumeTMAndVerified(account_polymath, I_MRProxied, I_PolyToken.address, web3.utils.toWei("500"));
        ...
        LockupLockupVolumeTransferManagerFactory:  
                                           ${I_LockupVolumeTransferManagerFactory.address}
        -----------------------------------------------------------------------------
        `);
    });
    ...
    describe("Buy tokens using on-chain whitelist and test locking them up and attempting to transfer", async() => {
        ...
        it("Should unsuccessfully attach the LockupVolumeTransferManager factory with the security token -- failed because Token is not paid", async () => {
            await I_PolyToken.getTokens(web3.utils.toWei("500", "ether"), token_owner);
            await catchRevert(
                 I_SecurityToken.addModule(P_LockupVolumeTransferManagerFactory.address, 0, web3.utils.toWei("500", "ether"), 0, { from: token_owner })
            )
        });

        it("Should successfully attach the LockupVolumeTransferManager factory with the security token", async () => {
            let snapId = await takeSnapshot();
            await I_PolyToken.transfer(I_SecurityToken.address, web3.utils.toWei("500", "ether"), {from: token_owner});
            const tx = await I_SecurityToken.addModule(P_LockupVolumeTransferManagerFactory.address, 0, web3.utils.toWei("500", "ether"), 0, { from: token_owner });
            assert.equal(tx.logs[3].args._types[0].toNumber(), transferManagerKey, "LockupVolumeTransferManagerFactory doesn't get deployed");
            assert.equal(
                web3.utils.toAscii(tx.logs[3].args._name)
                .replace(/\u0000/g, ''),
                "LockupVolumeRestrictionTM",
                "LockupVolumeTransferManagerFactory module was not added"
            );
            P_LockupVolumeTransferManager = LockupVolumeTransferManager.at(tx.logs[3].args._module);
            await revertToSnapshot(snapId);
        });

        it("Should successfully attach the LockupVolumeTransferManager with the security token", async () => {
            const tx = await I_SecurityToken.addModule(I_LockupVolumeTransferManagerFactory.address, 0, 0, 0, { from: token_owner });
            assert.equal(tx.logs[2].args._types[0].toNumber(), transferManagerKey, "LockupVolumeTransferManager doesn't get deployed");
            assert.equal(
                web3.utils.toAscii(tx.logs[2].args._name)
                .replace(/\u0000/g, ''),
                "LockupVolumeRestrictionTM",
                "LockupVolumeTransferManager module was not added"
            );
            
            I_LockupVolumeTransferManager = LockupVolumeTransferManager.at(tx.logs[2].args._module);
        });
        ...
        it("Should pause the tranfers at transferManager level", async() => {
            let tx = await I_LockupVolumeTransferManager.pause({from: token_owner});
        });
        ...
        it("Should unpause the tranfers at transferManager level", async() => {
            await I_LockupVolumeTransferManager.unpause({from: token_owner});
        });
        ...
        it("Should prevent the creation of a lockup with bad parameters where the totalAmount is zero", async() => {
            ...
            await catchRevert(
                I_LockupVolumeTransferManager.addLockUp(account_investor2, 16, 4, 2, 0, { from: token_owner })
            )
        });
        ...
        it("Should prevent the creation of a lockup with bad parameters where the releaseFrequencySeconds is zero", async() => {
            ...
            await catchRevert(
                I_LockupVolumeTransferManager.addLockUp(account_investor2, 16, 4, 0, web3.utils.toWei('1', 'ether'), { from: token_owner })
            );
        });
        ...
        it("Should prevent the creation of a lockup with bad parameters where the lockUpPeriodSeconds is zero", async() => {
            ...
            await catchRevert(
                I_LockupVolumeTransferManager.addLockUp(account_investor2, 4, 0, 4, web3.utils.toWei('1', 'ether'), { from: token_owner })
            );
        });
        ...
        it("Should prevent the creation of a lockup with bad parameters where the total amount to be released is more granular than allowed by the token", async() => {
            ...
            await catchRevert(
                I_LockupVolumeTransferManager.addLockUp(account_investor2, 0, 16, 4, web3.utils.toWei('0.5', 'ether'), { from: token_owner })
            );
        });
        ...
        it("Should prevent the creation of a lockup with bad parameters where the lockUpPeriodSeconds is not evenly divisible by releaseFrequencySeconds", async() => {
            ...
            await catchRevert(
                I_LockupVolumeTransferManager.addLockUp(account_investor2, 0, 17, 4, balance, { from: token_owner })
            );
        });
        ...
        it("Should prevent the creation of a lockup with bad parameters where the total amount being locked up isn't evenly divisible by the number of total periods", async() => {
            ...
            await catchRevert(
                I_LockupVolumeTransferManager.addLockUp(account_investor2, 0, web3.utils.toWei('16', 'ether'), 4, web3.utils.toWei('1', 'ether'), { from: token_owner })
            );
        });
        ...
        it("Should prevent the creation of a lockup with bad parameters where the amount to be released per period is too granular for the token", async() => {
            ...
            await catchRevert(
                I_LockupVolumeTransferManager.addLockUp(account_investor2, 0, 16, 4, balance, { from: token_owner })
            );
        ...
        it("Should prevent the transfer of tokens in a lockup", async() => {
            ...
            await I_LockupVolumeTransferManager.addLockUp(account_investor2, 0, 12, 4, balance, { from: token_owner });

            // read only - check if transfer will pass.  it should return INVALID
            let result = await I_LockupVolumeTransferManager.verifyTransfer.call(account_investor2, account_investor1, web3.utils.toWei('1', 'ether'), 0, false)
            // enum Result {INVALID, NA, VALID, FORCE_VALID} and we want VALID so it should be 2
            let tx = await I_LockupVolumeTransferManager.verifyTransfer(account_investor2, account_investor1, web3.utils.toWei('1', 'ether'), 0, true);
        ...
        it("Should prevent the transfer of tokens in an edited lockup", async() => {
            ...
            // create a lockup for their entire balance
            // over 16 seconds total, with 4 periods of 4 seconds each.
            await I_LockupVolumeTransferManager.addLockUp(account_investor1, 0, 16, 4,balance, { from: token_owner });
            ...
            let lockUpCount = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor1);
            assert.equal(lockUpCount, 1)

            let lockUp = await I_LockupVolumeTransferManager.getLockUp(account_investor1, 0);
            // console.log(lockUp);
            // elements in lockup array are uint lockUpPeriodSeconds, uint releaseFrequencySeconds, uint startTime, uint totalAmount
            assert.equal(lockUp[0].toNumber(), now);
            assert.equal(lockUp[1].toString(), '16');
            assert.equal(lockUp[2].toString(), '4');            
            assert.equal(lockUp[3].toString(), balance.toString());

            // edit the lockup
            await I_LockupVolumeTransferManager.modifyLockUp(account_investor1, 0, 0, 8, 4,balance, { from: token_owner });
        ...
        it("Should be possible to remove a lockup -- couldn't transfer because of lock up", async() => {
            ...
            // remove the lockup
            await I_LockupVolumeTransferManager.removeLockUp(account_investor1, 0, { from: token_owner });

           lockUpCount = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor1);
        ...
        it("Should be possible to create multiple lockups at once", async() => {
            ...
            lockUpCountsBefore[account_investor2] = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor2);
            assert.equal(lockUpCountsBefore[account_investor2], 1) // there's one old, expired lockup on acct already

            // get lockups for acct 3
            lockUpCountsBefore[account_investor3] = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor3);
            assert.equal(lockUpCountsBefore[account_investor3], 0)

            // create lockups for their entire balances
            await I_LockupVolumeTransferManager.addLockUpMulti(
            ...
            // get lockups for acct 2
            lockUpCountsAfter[account_investor2] = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor2);
            assert.equal(lockUpCountsAfter[account_investor2], 2);

            // get lockups for acct 3
            lockUpCountsAfter[account_investor3] = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor3);
        ...
        it("Should revert if the parameters are bad when creating multiple lockups", async() => {
            ...
            I_LockupVolumeTransferManager.addLockUpMulti(
                [account_investor2, account_investor3],
                [0, 0],
                [16, 8],
        ...
        it("Should be possible to create a lockup with a specific start time in the future", async() => {
            ...
            let lockUpsLength = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor2);
            assert.equal(lockUpsLength, 2);
            await I_LockupVolumeTransferManager.removeLockUp(account_investor2, 0, { from: token_owner });
            await I_LockupVolumeTransferManager.removeLockUp(account_investor2, 0, { from: token_owner });
            lockUpsLength = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor2);
            ...
            await I_LockupVolumeTransferManager.addLockUp(account_investor2, now + 4, 100, 10, balance, { from: token_owner });
        ...
        it("Should be possible to edit a lockup with a specific start time in the future", async() => {
            ...
            // check and get the lockup
            let lockUpCount = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor2);
            assert.equal(lockUpCount, 1)

            let lockUp = await I_LockupVolumeTransferManager.getLockUp(account_investor2, 0);

            // elements in lockup array are uint lockUpPeriodSeconds, uint releaseFrequencySeconds, uint startTime, uint totalAmount
            assert.isAtMost(lockUp[0].toNumber(), now);
            assert.equal(lockUp[1].toString(), '100');
            assert.equal(lockUp[2].toString(), '10');            
            assert.equal(lockUp[3].toString(), balance.toString());

            // edit the lockup
            await I_LockupVolumeTransferManager.modifyLockUp(account_investor2, 0, now + 4, 8, 4, balance, { from: token_owner });

            // check and get the lockup again
            lockUpCount = await I_LockupVolumeTransferManager.getLockUpsLength(account_investor2);
            assert.equal(lockUpCount, 1)

            lockUp = await I_LockupVolumeTransferManager.getLockUp(account_investor2, 0);

            // elements in lockup array are uint lockUpPeriodSeconds, uint releaseFrequencySeconds, uint startTime, uint totalAmount
            assert.isAtMost(lockUp[0].toNumber(), now + 4);
            assert.equal(lockUp[1].toString(), '8');
            assert.equal(lockUp[2].toString(), '4');
            assert.equal(lockUp[3].toString(), balance.toString());
            ...
            let lockUpBeforeVerify = await I_LockupVolumeTransferManager.getLockUp(account_investor2, 0);
            // check if transfer will pass in read-only operation
            let result = await I_LockupVolumeTransferManager.verifyTransfer.call(account_investor2, account_investor1, web3.utils.toWei('5', 'ether'), 0, false)
            // enum Result {INVALID, NA, VALID, FORCE_VALID} and we want VALID so it should be 2
            assert.equal(result.toString(), '2')
            let lockUpAfterVerify = await I_LockupVolumeTransferManager.getLockUp(account_investor2, 0);
        ...
        it("Should be possible to stack lockups", async() => {
            ...
            await I_LockupVolumeTransferManager.addLockUp(account_investor1, 0, 12, 4, web3.utils.toWei('6', 'ether'), { from: token_owner });
            ...
            await I_LockupVolumeTransferManager.addLockUp(account_investor1, 0, 16, 4, web3.utils.toWei('8', 'ether'), { from: token_owner });
        ...
        it("Should get configuration function signature", async() => {
            let sig = await I_LockupVolumeTransferManager.getInitFunction.call();
            assert.equal(web3.utils.hexToNumber(sig), 0);
        });


        it("Should get the permission", async() => {
            let perm = await I_LockupVolumeTransferManager.getPermissions.call();
    ...
    describe("VolumeRestriction Transfer Manager Factory test cases", async() => {
        ...
        it("Should get the exact details of the factory", async() => {
            assert.equal(await I_LockupVolumeTransferManagerFactory.setupCost.call(),0);
            assert.equal((await I_LockupVolumeTransferManagerFactory.getTypes.call())[0],2);
            assert.equal(web3.utils.toAscii(await I_LockupVolumeTransferManagerFactory.getName.call())
                        .replace(/\u0000/g, ''),
                        "LockupVolumeRestrictionTM",
                        "Wrong Module added");
            assert.equal(await I_LockupVolumeTransferManagerFactory.getDescription.call(),
                        "Manage transfers using lock ups over time",
                        "Wrong Module added");
            assert.equal(await I_LockupVolumeTransferManagerFactory.getTitle.call(),
                        "Lockup Volume Restriction Transfer Manager",
                        "Wrong Module added");
            assert.equal(await I_LockupVolumeTransferManagerFactory.getInstructions.call(),
        ...
        it("Should get the tags of the factory", async() => {
            let tags = await I_LockupVolumeTransferManagerFactory.getTags.call();
