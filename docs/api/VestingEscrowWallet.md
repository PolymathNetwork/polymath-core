...
```js
function sendToTreasury(uint256 _amount) external nonpayable withPerm 
```
...
```js
function pullAvailableTokens() external nonpayable
```
...
```js
function modifySchedule(address _beneficiary, bytes32 _templateName, uint256 _startTime) public nonpayable withPerm 
```
...
```js
function revokeSchedulesMulti(address[] _beneficiaries) external nonpayable withPerm 
```
