...
contract('Upgrade from v1.3.0 to v1.4.0', accounts => {
  ...
  describe('USDTieredSTOFactory deploy', async () => {
    ...
    it('Should successfully deploy POLY Oracle and register on PolymathRegistry', async () => {
      ...
      assert.notEqual(I_POLYOracle.address.valueOf(), '0x0000000000000000000000000000000000000000', 'POLYOracle contract was not deployed');
    ...
    it('Should successfully deploy ETH Oracle and register on PolymathRegistry', async () => {
      I_USDOracle = await ETHOracle.new('0x216d678c14be600cb88338e763bb57755ca2b1cf', '0x0000000000000000000000000000000000000000', 'ETH', {
        from: POLYMATH
      });
      assert.notEqual(I_USDOracle.address.valueOf(), '0x0000000000000000000000000000000000000000', 'USDOracle contract was not deployed');
