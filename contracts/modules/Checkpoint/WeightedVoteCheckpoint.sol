...
contract WeightedVoteCheckpoint is ICheckpoint {
    ...
    event VoteCasted(uint256 _ballotId, uint256 _time, address indexed _address, uint256 _weight, bool _vote);
    ...
    function getResults(uint256 _ballotId) public view returns (uint256 cummulativeYes, uint256 cummulativeNo, uint256 totalAbstain, uint256 remainingTime) {
        ...
        uint256 remainingTime = (ballots[_ballotId].endTime > now) ? ballots[_ballotId].endTime.sub(now) : 0;
        return (ballots[_ballotId].cumulativeYes, ballots[_ballotId].cumulativeNo, abstain, remainingTime);
    }
