...
contract('CappedSTO', accounts => {
    ...
    describe("Generate the SecurityToken", async() => {
        ...
        it("Should fail to launch the STO due to rate is 0", async () => {
            ...
            try {
            const tx = await I_SecurityToken.addModule(I_CappedSTOFactory.address, bytesSTO, 0, 0, true, { from: token_owner, gas: 26000000 });
            } catch(error) {
    ...
    describe("Buy tokens", async() => {
        ...
        it("Should unpause the STO -- Failed due to wrong msg.sender", async()=> {
            let errorThrown = false;
            try {
                let tx = await I_CappedSTO.unpause({from: account_investor1});
            } catch(error) {
                console.log(`Failed due to wrong msg.sender`);
                ensureException(error);
                errorThrown = true;
        ...
        it("Should unpause the STO", async()=> {
            let tx = await I_CappedSTO.unpause({from: account_issuer});
