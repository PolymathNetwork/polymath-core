...
contract("SecurityTokenRegistry", async (accounts) => {
    ...
    describe(" Test cases of the registerTicker", async () => {
        ...
        it("Should fail to register ticker if tickerRegFee not approved", async () => {
            await catchRevert(
                I_STRProxied.registerTicker(account_temp, symbol, { from: account_temp }),
        ...
        it("Should fail to register ticker if owner is 0x", async () => {
            ...
            await catchRevert(
                I_STRProxied.registerTicker(address_zero, symbol, { from: account_temp }),
                "tx revert -> owner should not be 0x"
            );
        });

        it("Should fail to register ticker due to the symbol length is 0", async () => {
            await catchRevert(I_STRProxied.registerTicker(account_temp, "", { from: account_temp }), "tx revert -> Symbol Length is 0");
        });

        it("Should fail to register ticker due to the symbol length is greater than 10", async () => {
            await catchRevert(
                I_STRProxied.registerTicker(account_temp, "POLYMATHNET", { from: account_temp }),
                "tx revert -> Symbol Length is greater than 10"
            );
        });

        it("Should register the ticker before the generation of the security token", async () => {
            let tx = await I_STRProxied.registerTicker(account_temp, symbol, { from: account_temp });
        ...
        it("Should fail to register same symbol again", async () => {
            ...
            await catchRevert(
                I_STRProxied.registerTicker(token_owner, symbol, { from: token_owner }),
        ...
        it("Should fail to register ticker if registration is paused", async () => {
            ...
            await catchRevert(
                I_STRProxied.registerTicker(token_owner, "AAA", { from: token_owner }),
        ...
        it("Should successfully register ticker if registration is unpaused", async () => {
            ...
            await I_PolyToken.approve(I_STRProxied.address, initRegFeePOLY, { from: token_owner });
            let tx = await I_STRProxied.registerTicker(token_owner, "AAA", { from: token_owner });
            assert.equal(tx.logs[0].args._owner, token_owner, `Owner should be the ${token_owner}`);
    ...
    describe("Generate SecurityToken", async () => {
        ...
        it("Should fail to generate the SecurityToken because ticker gets expired", async () => {
            ...
            let tx = await I_STRProxied.registerTicker(token_owner, "CCC", { from: token_owner });
        ...
        it("Should get all created security tokens", async() => {
            ...
            await I_STRProxied.registerTicker(account_temp, "TMP", { from: account_temp });
    ...
    describe("Generate custom tokens", async () => {
        ...
        it("Should fail to genrate the custom security token -- ticker length is greater than 10 chars", async () => {
            await catchRevert(
                I_STRProxied.modifySecurityToken("LOGLOGLOGLOG", account_temp, dummy_token, "I am custom ST", currentTime, {
                    from: account_polymath
                }),
        ...
        it("Should fail if ST address is 0 address", async () => {
            await catchRevert(
                I_STRProxied.modifySecurityToken("LOG", account_temp, address_zero, "I am custom ST", currentTime, {
                    from: account_polymath
                }),
        ...
        it("Should fail to generate the custom ST -- deployedAt param is 0", async () => {
            await catchRevert(
                I_STRProxied.modifySecurityToken(symbol2, token_owner, dummy_token, "I am custom ST", new BN(0), { from: account_polymath }),
        ...
        it("Should successfully generate custom token", async () => {
            ...
            await I_STRProxied.registerTicker(account_temp, "LOG", { from: account_temp });
            tickersListArray = await I_Getter.getTickersByOwner.call(account_temp);
            console.log(tickersListArray);
            // Generating the ST
            let tx = await I_STRProxied.modifySecurityToken("LOG", account_temp, dummy_token, "I am custom ST", currentTime, {
                from: account_polymath
            });
            tickersListArray = await I_Getter.getTickersByOwner.call(account_temp);
            console.log(tickersListArray);
        ...
        it("Should successfully generate the custom token", async () => {
            ...
            //
            // await catchRevert(I_STRProxied.modifySecurityToken("LOG2", account_temp, dummy_token, "I am custom ST", await latestTime(), {from: account_polymath}));
            // await I_STRProxied.modifyTicker(account_temp, "LOG2", await latestTime(), currentTime.add(new BN(duration.days(10))), false, {from: account_polymath});
            // await increaseTime(duration.days(1));
            let tx = await I_STRProxied.modifySecurityToken("LOG2", account_temp, dummy_token, "I am custom ST", currentTime, {
                from: account_polymath
            });
    ...
    describe("Test case for modifyTicker", async () => {
        it("Should add the custom ticker --failed because of bad owner", async () => {
            currentTime = new BN(await latestTime());
            await catchRevert(
                I_STRProxied.modifyTicker(token_owner, "ETH", currentTime, currentTime.add(new BN(duration.days(10))), false, {
                    from: account_temp
                }),
        ...
        it("Should add the custom ticker --fail ticker length should not be 0", async () => {
            await catchRevert(
                I_STRProxied.modifyTicker(token_owner, "", currentTime, currentTime.add(new BN(duration.days(10))), false, {
                    from: account_polymath
                }),
        ...
        it("Should add the custom ticker --failed because time should not be 0", async () => {
            await catchRevert(
                I_STRProxied.modifyTicker(token_owner, "ETH", new BN(0), currentTime.add(new BN(duration.days(10))), false, {
                    from: account_polymath
                }),
        ...
        it("Should add the custom ticker --failed because registeration date is greater than the expiryDate", async () => {
            ...
            await catchRevert(
                I_STRProxied.modifyTicker(token_owner, "ETH", ctime, ctime.sub(new BN(duration.minutes(10))), false, {
                    from: account_polymath
                }),
    ...
    describe("Test cases for the changeTickerRegistrationFee()", async () => {
        ...
        it("Should fail to register the ticker with the old fee", async () => {
            await I_PolyToken.approve(I_STRProxied.address, initRegFeePOLY, { from: token_owner });
            await catchRevert(
                I_STRProxied.registerTicker(token_owner, "POLY", { from: token_owner }),
        ...
        it("Should register the ticker with the new fee", async () => {
            ...
            await I_PolyToken.approve(I_STRProxied.address, new BN(web3.utils.toWei("2000")), { from: token_owner });
            let tx = await I_STRProxied.registerTicker(token_owner, "POLY", { from: token_owner });
            assert.equal(tx.logs[0].args._owner, token_owner, `Token owner should be ${token_owner}`);
