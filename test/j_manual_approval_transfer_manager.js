...
contract('ManualApprovalTransferManager', accounts => {
  ...
  before(async () => {
    ...
    P_ManualApprovalTransferManagerFactory = await ManualApprovalTransferManagerFactory.new(
      I_PolyToken.address,
  ...
  describe('Buy tokens using whitelist & manual approvals', async () => {
    ...
    it('Cannot call verifyTransfer on the TM directly if _isTransfer == true', async () => {
      await catchRevert(
        I_ManualApprovalTransferManager.verifyTransfer(account_investor4, account_investor4, web3.utils.toWei('2', 'ether'), true, {
          from: token_owner
        })
      );
    });
    ...
    it('Can call verifyTransfer on the TM directly if _isTransfer == false', async () => {
      await I_ManualApprovalTransferManager.verifyTransfer(account_investor4, account_investor4, web3.utils.toWei('2', 'ether'), false, {
        from: token_owner
      });
    });
    ...
    it('Should fail to add a manual approval because invalid expiry time', async () => {
      await catchRevert(
        I_ManualApprovalTransferManager.addManualApproval(account_investor1, account_investor4, web3.utils.toWei('2', 'ether'), 99999, {
          from: token_owner
        })
      );
    });
    ...
    it('Should fail to add a manual block because invalid _from address', async () => {
      await catchRevert(
        I_ManualApprovalTransferManager.addManualBlocking('', account_investor2, latestTime() + duration.days(1), { from: token_owner })
      );
    });

    it('Should fail to add a manual block because invalid _to address', async () => {
      await catchRevert(
        I_ManualApprovalTransferManager.addManualBlocking(account_investor1, '', latestTime() + duration.days(1), { from: token_owner })
      );
    });
