...
contract('USDTieredSTO Sim', accounts => {
  ...
  before(async () => {
    ...
    I_USDTieredSTOFactory = await USDTieredSTOFactory.new(I_PolyToken.address, STOSetupCost, 0, 0, I_USDTieredSTOProxyFactory.address, {
      from: ISSUER
    });
  ...
  describe('Deploy the STO', async () => {
    ...
    it('Should successfully attach the first STO module to the security token', async () => {
      ...
      _ratePerTier.push([BigNumber(0.05 * 10 ** 18), BigNumber(0.13 * 10 ** 18), BigNumber(0.17 * 10 ** 18)]); // [ 0.05 USD/Token, 0.10 USD/Token, 0.15 USD/Token ]
  ...
  describe('Simulate purchasing', async () => {
    it('Should successfully complete simulation', async () => {
      ...
      async function invest(_investor, _isAccredited) {
        ...
        for (var i = 0; i < _ratePerTier[stoId].length; i++) {
          Tokens_total.push(
            (await I_USDTieredSTO_Array[stoId].tokensPerTierTotal.call(i)).sub(await I_USDTieredSTO_Array[stoId].mintedPerTierTotal.call(i))
          );
      ...
      async function processInvestment(
        ...
        if (isPoly && investment_POLY.gt(10)) {
          ...
          console.log(
            `buyWithPOLY: ${investment_Token.div(10 ** 18)} tokens for ${investment_POLY.div(10 ** 18)} POLY by ${_investor}`.yellow
          );
          ...
          gasCost = BigNumber(GAS_PRICE).mul(tx.receipt.gasUsed);
          console.log(
            `buyWithUSD: ${investment_Token.div(10 ** 18)} tokens for ${investment_DAI.div(10 ** 18)} DAI by ${_investor}`.yellow
          );
          ...
          gasCost = BigNumber(GAS_PRICE).mul(tx.receipt.gasUsed);
          console.log(
            `buyWithETH: ${investment_Token.div(10 ** 18)} tokens for ${investment_ETH.div(10 ** 18)} ETH by ${_investor}`.yellow
          );
        }
