...
contract("SecurityToken", async (accounts) => {
    ...
    describe("Generate the SecurityToken", async () => {
        ...
        it("Should check the balance of the locked tokens", async() => {
            ...
            console.log(`\t Locked balance: ${web3.utils.fromWei((await stGetter.balanceOfByPartition.call(web3.utils.utf8ToHex(`LOCKED`), account_affiliate1)).toString())}`);
            console.log(`\t Unlocked balance: ${web3.utils.fromWei((await stGetter.balanceOfByPartition.call(web3.utils.utf8ToHex(`UNLOCKED`), account_affiliate1)).toString())}`);
            assert.equal(
                web3.utils.fromWei((await stGetter.balanceOfByPartition.call(web3.utils.utf8ToHex(`LOCKED`), account_affiliate1)).toString()),
                0
            );
            assert.equal(
                web3.utils.fromWei((await stGetter.balanceOfByPartition.call(web3.utils.utf8ToHex(`UNLOCKED`), account_affiliate1)).toString()),
                web3.utils.fromWei((await I_SecurityToken.balanceOf.call(account_affiliate1)).toString())
            );
            console.log(`\t Wrong partition: ${web3.utils.fromWei((await stGetter.balanceOfByPartition.call(web3.utils.toHex(`OCKED`), account_affiliate1)).toString())}`);
            assert.equal(
                web3.utils.fromWei((await stGetter.balanceOfByPartition.call(web3.utils.toHex(`OCKED`), account_affiliate1)).toString()),
                0
            );
        });
