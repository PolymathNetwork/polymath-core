...
const GeneralPermissionManager = artifacts.require(
...
contract("PLCRVotingCheckpoint", async accounts => {
  ...
  before(async () => {
    ...
    [I_PLCRVotingCheckpointFactory] = await deployPLCRVoteCheckpoint(
      account_polymath,
      I_MRProxied,
      0
    );
    [P_PLCRVotingCheckpointFactory] = await deployPLCRVoteCheckpoint(
      account_polymath,
      I_MRProxied,
    ...
    console.log(`
        ...
        PLCRVotingCheckpointFactory:     ${
          I_PLCRVotingCheckpointFactory.address
        }
        -----------------------------------------------------------------------------
        `);
  });
  ...
  describe("\t\t Test case for PLCR Vote Checkpoint module \n", async () => {
    describe("\t\t Attaching the PLCR vote checkpoint module \n", async () => {
      it("\t\t Should register the ticker before the generation of the security token \n", async () => {
        ...
        let tx = await I_STRProxied.registerTicker(token_owner, symbol, name, {
          from: token_owner
        });
      ...
      it("\t\t Should generate the new security token with the same symbol as registered above \n", async () => {
        ...
        let tx = await I_STRProxied.generateNewSecurityToken(
          name,
          symbol,
          tokenDetails,
          false,
        ...
        assert.equal(
          tx.logs[1].args._ticker,
          symbol,
          "SecurityToken doesn't get deployed"
        );

        I_SecurityToken = await SecurityToken.at(
          tx.logs[1].args._securityTokenAddress
        );
        stGetter = await STGetter.at(I_SecurityToken.address);
        const log = (await I_SecurityToken.getPastEvents("ModuleAdded", {
          filter: { transactionHash: tx.transactionHash }
        }))[0];
        // Verify that GeneralTransferManager module get added successfully or not
        assert.equal(log.args._types[0].toNumber(), transferManagerKey);
        assert.equal(
          web3.utils.hexToString(log.args._name),
          "GeneralTransferManager"
        );
      });

      it("\t\t Should intialize the auto attached modules \n", async () => {
        let moduleData = (await stGetter.getModulesByType(
          transferManagerKey
        ))[0];
      ...
      it("\t\t Should attach the voting module with the ST \n", async () => {
        ...
        assert.equal(
          tx.logs[2].args._types[0],
          checkpointKey,
          "Checkpoint doesn't get deployed"
        );
        assert.equal(
          web3.utils.hexToString(tx.logs[2].args._name),
          "PLCRVotingCheckpoint",
          "PLCRVotingCheckpoint module was not added"
        );
        I_PLCRVotingCheckpoint = await PLCRVotingCheckpoint.at(
          tx.logs[2].args._module
        );
      });

      it("\t\t Should fail to attach the voting module because allowance is unsufficent \n", async () => {
        await catchRevert(
          I_SecurityToken.addModule(
            P_PLCRVotingCheckpointFactory.address,
            "0x0",
            new BN(web3.utils.toWei("500")),
            0,
            false,
      ...
      it("\t\t Should attach the voting module with the ST \n", async () => {
        ...
        assert.equal(
          tx.logs[3].args._types[0],
          checkpointKey,
          "Checkpoint doesn't get deployed"
        );
        assert.equal(
          web3.utils.hexToString(tx.logs[3].args._name),
          "PLCRVotingCheckpoint",
          "PLCRVotingCheckpoint module was not added"
        );
      ...
      it("\t\t Should attach the general permission manager", async () => {
        ...
        assert.equal(
          tx.logs[2].args._types[0],
          delegateManagerKey,
          "Permission manager doesn't get deployed"
        );
        assert.equal(
          web3.utils.hexToString(tx.logs[2].args._name),
          "GeneralPermissionManager",
        ...
        I_GeneralPermissionManager = await GeneralPermissionManager.at(
          tx.logs[2].args._module
        );
      });
    });
    ...
    describe("\t\t Test for createBallot \n", async () => {
      it("\t\t Should fail to create ballot -- bad owner \n", async () => {
        await catchRevert(
          I_PLCRVotingCheckpoint.createBallot(
            new BN(duration.days(5)),
            new BN(duration.days(10)),
            new BN(3),
            new BN(46.57).mul(new BN(10).pow(new BN(16))),
      ...
      it("\t\t Mint some tokens and transfer to whitelisted investors \n", async () => {
        ...
        assert.equal(
          web3.utils.fromWei(
            (await I_SecurityToken.balanceOf.call(account_investor1)).toString()
          ),
          500
        );
        assert.equal(
        ...
        assert.equal(
          web3.utils.fromWei(
            (await I_SecurityToken.balanceOf.call(account_investor3)).toString()
          ),
          5000
        );
      });
      ...
      it("\t\t Should fail to create ballot -- Invalid checkpoint Id \n", async () => {
        ...
        await catchRevert(
          I_PLCRVotingCheckpoint.createCustomBallot(
            commitTime,
            revealTime,
            new BN(3),
            new BN(50).mul(new BN(10).pow(new BN(16))),
            new BN(45),
            startTime,
            { from: token_owner }
          )
      ...
      it("\t\t SHould give admin permission to the delegate \n", async () => {
        await I_GeneralPermissionManager.addDelegate(
          account_delegate,
          web3.utils.toHex("I am a delegate"),
          { from: token_owner }
        );
        await I_GeneralPermissionManager.changePermission(
          account_delegate,
          I_PLCRVotingCheckpoint.address,
      ...
      it("\t\t Should create the ballot successfully \n", async () => {
        let startTime = new BN(await latestTime()).add(
          new BN(duration.minutes(5))
        );
        let commitTime = new BN(duration.days(4));
        let revealTime = new BN(duration.days(5));
        await I_SecurityToken.createCheckpoint({ from: token_owner });
        let checkpointId = await I_SecurityToken.currentCheckpointId.call();
        let tx = await I_PLCRVotingCheckpoint.createCustomBallot(
          commitTime,
          revealTime,
          new BN(3),
          new BN(47.8).mul(new BN(10).pow(new BN(16))),
          checkpointId,
          startTime,
    ...
    describe("\t\t Test case for commitVote \n", async () => {
      it("\t\t Should fail to commitVote -- bad ballot id \n", async () => {
        await catchRevert(
          I_PLCRVotingCheckpoint.commitVote(
            new BN(2),
            web3.utils.toHex("Some secret"),
      ...
      it("\t\t Should fail to commitVote -- not in the commit stage \n", async () => {
        ...
        await catchRevert(
          I_PLCRVotingCheckpoint.commitVote(
            new BN(0),
            web3.utils.soliditySha3(1, salt),
            { from: account_investor1 }
          )
        );
      });
      ...
      it("\t\t Should fail to commitVote -- secret vote is 0 \n", async () => {
        ...
        await catchRevert(
          I_PLCRVotingCheckpoint.commitVote(new BN(0), "0x0", {
            from: account_investor1
          })
        );
      });
      ...
      it("\t\t Should change some ballot status \n", async () => {
        await I_PLCRVotingCheckpoint.changeBallotStatus(new BN(0), false, {
          from: token_owner
        });
        let data = await I_PLCRVotingCheckpoint.getBallotStats.call(new BN(0));
        assert.isFalse(data[8]);
      });
      ...
      it("\t\t Should change some ballot status \n", async () => {
        await I_PLCRVotingCheckpoint.changeBallotStatus(new BN(0), true, {
          from: token_owner
        });
        let data = await I_PLCRVotingCheckpoint.getBallotStats.call(new BN(0));
        assert.isTrue(data[8]);
      });

      it("\t\t Should successfully vote by account investor1 \n", async () => {
        let salt = getRandom();
        saltArray.push(salt);
        let tx = await I_PLCRVotingCheckpoint.commitVote(
          new BN(0),
          web3.utils.soliditySha3(2, salt),
          { from: account_investor1 }
        );
        assert.equal(tx.logs[0].args._ballotId, 0);
        assert.equal(
          tx.logs[0].args._secretVote,
          web3.utils.soliditySha3(2, salt)
        );
      ...
      it("\t\t Mint some more tokens and transferred to the tokens holders \n", async () => {
        ...
        assert.equal(
          web3.utils.fromWei(
            (await I_SecurityToken.balanceOf.call(account_investor4)).toString()
          ),
          3500
        );
      });
      ...
      it("\t\t Should create a new ballot \n", async () => {
        ...
        let tx = await I_PLCRVotingCheckpoint.createBallot(
          commitTime,
          revealTime,
          new BN(4),
          new BN(51).mul(new BN(10).pow(new BN(16))),
          { from: account_delegate }
        );
      ...
      it("\t\t Should reveal the vote -- failed because not a valid stage \n", async () => {
        await catchRevert(
          I_PLCRVotingCheckpoint.revealVote(
            new BN(0),
            new BN(0),
            saltArray[1],
            { from: account_investor3 }
      ...
      it("\t\t Should successfully reveal the vote by investor 3 \n", async () => {
        let tx = await I_PLCRVotingCheckpoint.revealVote(
          new BN(0),
          new BN(1),
          saltArray[1],
          { from: account_investor3 }
      ...
      it("\t\t Should fail to change the ballot status-- bad owner \n", async () => {
        await catchRevert(
          I_PLCRVotingCheckpoint.changeBallotStatus(new BN(0), false, {
            from: account_polymath
          })
        );
      });
      ...
      it("\t\t Should fail to change the ballot status-- no change in the state \n", async () => {
        await catchRevert(
          I_PLCRVotingCheckpoint.changeBallotStatus(new BN(0), true, {
            from: token_owner
          })
        );
      });
      ...
      it("\t\t Should change the status of the ballot with the help of changeBallotStatus \n", async () => {
        let tx = await I_PLCRVotingCheckpoint.changeBallotStatus(
          new BN(0),
          false,
          { from: token_owner }
        );
      ...
      it("\t\t Should successfully reveal the vote by investor 1 \n", async () => {
        ...
        let txData = await I_PLCRVotingCheckpoint.revealVote(
          new BN(0),
          new BN(2),
          saltArray[0],
          { from: account_investor1 }
      ...
      it("\t\t Should ge the ballot stage \n", async () => {
        let stage1 = await I_PLCRVotingCheckpoint.getCurrentBallotStage.call(
          new BN(0)
        );
        assert.equal(stage1.toString(), 2);
        let stage2 = await I_PLCRVotingCheckpoint.getCurrentBallotStage.call(
          new BN(1)
        );
      ...
      it("\t\t Should check who votes whom \n", async () => {
        assert.equal(
          (await I_PLCRVotingCheckpoint.getSelectedProposal.call(
            new BN(0),
            account_investor1
          )).toString(),
          2
      ...
      it("\t\t Should give the result to 0 because ballot id is not valid", async () => {
        let data = await I_PLCRVotingCheckpoint.getBallotResults.call(
          new BN(5)
        );
    ...
    describe("\t\t General function test \n", async () => {
      ...
      it("\t\t Should check the init function \n", async () => {
        assert.equal(
          await I_PLCRVotingCheckpoint.getInitFunction.call(),
          "0x00000000"
        );
      });
    });
    ...
    describe("\t\t Factory test cases \n", async () => {
      it("\t\t Should get the exact details of the factory \n", async () => {
        assert.equal(
          (await I_PLCRVotingCheckpointFactory.setupCost.call()).toNumber(),
          0
        );
        assert.equal((await I_PLCRVotingCheckpointFactory.types.call())[0], 4);
        assert.equal(
          await I_PLCRVotingCheckpointFactory.version.call(),
          "3.0.0"
        );
        assert.equal(
          web3.utils
            .toAscii(await I_PLCRVotingCheckpointFactory.name.call())
            .replace(/\u0000/g, ""),
          "PLCRVotingCheckpoint",
          "Wrong Module added"
        );
        ...
        assert.equal(
          await I_PLCRVotingCheckpointFactory.title.call(),
          "PLCR Voting Checkpoint",
          "Wrong Module added"
        );
        let tags = await I_PLCRVotingCheckpointFactory.tags.call();
