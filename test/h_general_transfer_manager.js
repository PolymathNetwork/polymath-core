...
contract('GeneralTransferManager', accounts => {
  ...
  describe('Generate the SecurityToken', async () => {
    ...
    it('Should successfully attach the STO factory with the security token', async () => {
      ...
      assert.equal(web3.utils.toAscii(tx.logs[2].args._name).replace(/\u0000/g, ''), 'DummySTO', 'DummySTOFactory module was not added');
  ...
  describe('Buy tokens using off-chain whitelist', async () => {
    ...
    it('Should buy the tokens -- Failed due to incorrect signature input', async () => {
      ...
      await catchRevert(
        I_GeneralTransferManager.modifyWhitelistSigned(account_investor2, fromTime, toTime, expiryTime, true, validFrom, validTo, v, r, s, {
  ...
  describe('Dummy STO Factory test cases', async () => {
    it('should get the exact details of the factory', async () => {
      ...
      assert.equal(web3.utils.toAscii(await I_DummySTOFactory.getName.call()).replace(/\u0000/g, ''), 'DummySTO', 'Wrong Module added');
