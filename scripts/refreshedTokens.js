...
const web3 = new Web3(new Web3.providers.HttpProvider("https://mainnet.infura.io/v3/d54358ee83d14ea38ec158a35b0cd246"));
...
const strABI = JSON.parse(
    require("fs")
        .readFileSync("../build/contracts/SecurityTokenRegistry.json")
        .toString()
).abi;
...
// console.log(strABI)
//     const str = new web3.eth.Contract(strABI, securityTokenRegistryAddress);
//     const events = await str.getPastEvents('SecurityTokenRefreshed', {
//         fromBlock: 0,
//         toBlock: 'latest'});
//     console.log('events', events.length)
    let logs1 = await getLogsFromEtherscan(
        securityTokenRegistry.options.address,
        0,
        "latest",
        "NewSecurityToken(string,string,address,address,uint256,address,bool,uint256)"
    );
    let logs2 = await getLogsFromEtherscan(
        securityTokenRegistry.options.address,
        0,
        "latest",
        "NewSecurityToken(string,string,address,address,uint256,address,bool,uint256,uint256,uint256)"
    );
    let logs3 = await getLogsFromEtherscan(
        securityTokenRegistry.options.address,
        0,
        "latest",
        "SecurityTokenRefreshed(string,string,address,address,uint256,address,uint256)"
    );
    ...
    for (let i = 0; i < logs.length; i++) {
        ...
        console.log(await getInfo(tokenAddress));
    }
    // console.log(tokens);
}
