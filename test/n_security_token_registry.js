...
contract('SecurityTokenRegistry', accounts => {
  ...
  describe(' Test cases of the registerTicker', async () => {
    ...
    it('Should fail to register ticker if registration is paused', async () => {
      ...
      catchRevert(I_STRProxied.registerTicker(token_owner, 'AAA', name, { from: token_owner }), 'tx revert -> Registration is paused');
    });
  ...
  describe('Test case for modifyTicker', async () => {
    ...
    it('Should add the new custom ticker', async () => {
      let tx = await I_STRProxied.modifyTicker(account_temp, 'ETH', 'Ether', latestTime(), latestTime() + duration.minutes(10), false, {
        from: account_polymath
      });
    ...
    it('Should change the details of the existing ticker', async () => {
      let tx = await I_STRProxied.modifyTicker(token_owner, 'ETH', 'Ether', latestTime(), latestTime() + duration.minutes(10), false, {
        from: account_polymath
      });
  ...
  describe('Test cases for the changeExpiryLimit()', async () => {
    it('Should able to change the ExpiryLimit-- failed because of bad owner', async () => {
      catchRevert(I_STRProxied.changeExpiryLimit(duration.days(15), { from: account_temp }), 'tx revert -> failed because of bad owner');
    });
  ...
  describe('Test case for the update poly token', async () => {
    it('Should change the polytoken address -- failed because of bad owner', async () => {
      catchRevert(I_STRProxied.updatePolyTokenAddress(dummy_token, { from: account_temp }), 'tx revert -> failed because of bad owner');
    });
  ...
  describe(' Test cases of the registerTicker', async () => {
    ...
    it('Should modify ticker 1', async () => {
      let tx = await I_STRProxied.modifyTicker(account_temp, 'TOK1', 'TOKEN 1', latestTime(), latestTime() + duration.minutes(10), false, {
        from: account_polymath
      });
      assert.equal(tx.logs[0].args._owner, account_temp, `Should be equal to the ${account_temp}`);
    ...
    it('Should modify ticker 3', async () => {
      let tx = await I_STRProxied.modifyTicker(account_temp, 'TOK3', 'TOKEN 3', latestTime(), latestTime() + duration.minutes(10), false, {
        from: account_polymath
      });
      assert.equal(tx.logs[0].args._owner, account_temp, `Should be equal to the ${account_temp}`);
