...
contract("RestrictedPartialSaleTM", accounts => {
    ...
    before(async () => {
        ...
        [I_RestrictedPartialSaleTMFactory] = await deployRestrictedPartialSaleTMAndVerifyed(account_polymath, I_MRProxied, I_PolyToken.address, 0);
        // STEP 3: Deploy Paid the RestrictedPartialSaleTM
        [P_RestrictedPartialSaleTMFactory] = await deployRestrictedPartialSaleTMAndVerifyed(account_polymath, I_MRProxied, I_PolyToken.address, web3.utils.toWei("500", "ether"));
    ...
    describe("Generate the SecurityToken", async () => {
        ...
        it("Should fali to attach the RestrictedPartialSaleTM factory with the security token -- Insufficent balance", async () => {
            ...
            await catchRevert(
                I_SecurityToken.addModule(P_RestrictedPartialSaleTMFactory.address, bytesSale, web3.utils.toWei("500", "ether"), 0, {
                    from: token_owner
                })
            );
        });
        ...
        it("Should successfully attach the RestrictedPartialSaleTM with the security token", async () => {
            const tx = await I_SecurityToken.addModule(I_RestrictedPartialSaleTMFactory.address, bytesSale, 0, 0, { from: token_owner });
            ...
            I_RestrictedPartialSaleTM = RestrictedPartialSaleTM.at(tx.logs[2].args._module);
        });
    });
    ...
    describe("Whitelist the addresses", async () => {
        ...
        it("Should whitelist the investor 3 and invest some ETH in STO to buy some tokens", async() => {
            ...
            await I_DummySTO.generateTokens(account_investor3, web3.utils.toWei("1000"), {from: token_owner});

            assert.equal((await I_SecurityToken.balanceOf(account_investor3)).toNumber(), web3.utils.toWei("1000"));
        });

        it("Should failed to transafer the partial token balance", async() => {
            await catchRevert(
                I_SecurityToken.transfer(account_investor2, web3.utils.toWei("10"), {from: account_investor3})
            );
        });
        ...
        it("Should pause the RestrictedPartialSaleTM and sale partial balance", async() => {
            ...
            await I_SecurityToken.transfer(account_investor2, web3.utils.toWei("10"), {from: account_investor3});
        ...
        it("Should whitelist the treasury wallet address", async() => {
            ...
            await I_SecurityToken.mint(account_treasury, web3.utils.toWei("1000000"), { from: token_owner });
        ...
        it("Should send partial balance successfully", async() => {
            ...
            await I_SecurityToken.transfer(account_investor1, web3.utils.toWei("500"), {from: account_treasury});
            assert.equal(
    ...
    describe("Test for the exemption list", async() => {
        ...
        it("Should sale the tokens freely", async() => {
            await I_SecurityToken.transfer(account_investor1, web3.utils.toWei("5"), {from: account_investor2});
            assert.equal(
        ...
        it("Should sale tokens freely", async() => {
            await I_SecurityToken.transfer(account_investor1, web3.utils.toWei("1"), {from: account_investor2});
            await I_SecurityToken.transfer(account_investor3, web3.utils.toWei("1"), {from: account_investor2});
            await I_SecurityToken.transfer(account_investor2, web3.utils.toWei("1"), {from: account_investor1});
            await I_SecurityToken.transfer(account_investor3, web3.utils.toWei("1"), {from: account_investor1});
            await I_SecurityToken.transfer(account_investor2, web3.utils.toWei("1"), {from: account_investor3});
        ...
        it("Should change the exemption list", async() => {
            ...
            await catchRevert(
                I_SecurityToken.transfer(account_investor1, web3.utils.toWei("1"), {from: account_investor2})
            );
        });
        ...
        it("Should change the exemption list multi", async() => {
            ...
            await catchRevert(
                I_SecurityToken.transfer(account_investor1, web3.utils.toWei("1"), {from: account_investor3})
            );
        });
    ...
    describe("Test cases for the RestrictedPartialSaleTMFactory", async () => {
        it("should get the exact details of the factory", async () => {
            assert.equal((await I_RestrictedPartialSaleTMFactory.getSetupCost.call()).toNumber(), 0);
            assert.equal((await I_RestrictedPartialSaleTMFactory.getTypes.call())[0], 2);
            assert.equal(await I_RestrictedPartialSaleTMFactory.version.call(), "2.2.0");
            assert.equal(
                web3.utils.toAscii(await I_RestrictedPartialSaleTMFactory.getName.call()).replace(/\u0000/g, ""),
                "RestrictedPartialSaleTM",
                "Wrong Module added"
            );
            assert.equal(
            ...
            assert.equal(
                await I_RestrictedPartialSaleTMFactory.getInstructions.call(),
                "Allows an issuer to restrict the investors to sale partial token balances",
                "Wrong Module added"
            );
