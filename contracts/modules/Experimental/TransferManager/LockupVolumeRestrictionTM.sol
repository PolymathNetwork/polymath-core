...
contract LockupVolumeRestrictionTM is ITransferManager {
     ...
     */
    function removeLockUpMulti(address[] _userAddresses) external withPerm(ADMIN) {
        require(_userAddresses.length > 0, "Array length mismatch");
        for (uint i = 0; i < _userAddresses.length; i++) {
            removeLockUp(_userAddresses[i]);
        }
    }
    ...
    function modifyLockUp(
        ...
        uint256 startTime = _startTime;
        ///require(_totalAmount >= lockUps[_userAddress].alreadyWithdrawn, "Total amount should be >= already withdrawn amount");
        // if a startTime of 0 is passed in, then start now.
        if (startTime == 0) {
            /*solium-disable-next-line security/no-block-members*/
            startTime = now;
     ...
     * @param _userAddress Address of the user whose tokens should be locked up
     */
    function getLockUp(address _userAddress) public view returns
    (
        uint lockUpPeriodSeconds,
        uint releaseFrequencySeconds,
        uint startTime,
