...
contract('BlacklistTransferManager', accounts => {
    ...
    describe("Generate the SecurityToken", async() => {
        ...
        it("Should successfully attach the BlacklistTransferManager factory with the security token", async () => {
            ...
            const tx = await I_SecurityToken.addModule(P_BlacklistTransferManagerFactory.address, bytesSTO, web3.utils.toWei("500", "ether"), 0, { from: token_owner });
            assert.equal(tx.logs[3].args._type.toNumber(), transferManagerKey, "BlacklistTransferManager doesn't get deployed");
            assert.equal(
        ...
        it("Should successfully attach the BlacklistTransferManager with the security token", async () => {
            ...
            assert.equal(tx.logs[2].args._type.toNumber(), transferManagerKey, "BlacklistTransferManager doesn't get deployed");
            assert.equal(
    ...
    describe("Buy tokens using on-chain whitelist", async() => {
        ...
        it("Should fail in adding the blacklist", async() => {
            let errorThrown = false;
            try {
        ...
        it("Should fail in adding the existing blacklist", async() => {
            let errorThrown = false;
            try {
                await I_BlacklistTransferManager.addBlacklistType(latestTime()+1000, latestTime()+3000, "a_blacklist", 20, { from: token_owner });
            } catch(error) {
        ...
        it("Should fail in adding the investor to the blacklist", async() => {
            ...
            try {
                ...
                console.log(`         tx revert -> Investor already associated to blacklist`.grey);
                errorThrown = true;
                ensureException(error);
            }
            assert.ok(errorThrown, message);
        ...
        it("Should fail in getting the list of investors associated to blacklist", async() => {
            let errorThrown = false;
            try{
        ...
        it("Should fail in deleting the blacklist type", async() => {
            await I_BlacklistTransferManager.addBlacklistType(latestTime()+1000, latestTime()+3000, "b_blacklist", 20, { from: token_owner });
            let errorThrown = false;
            try {
        ...
        it("Should delete the investor from the blacklist type", async() => {
            let tx = await I_BlacklistTransferManager.deleteInvestorFromBlacklist(account_investor1, { from: token_owner });
        ...
        it("Should fail in deleting the investor from the blacklist type", async() => {
            await I_BlacklistTransferManager.addInvestorToBlacklist(account_investor1, "a_blacklist", { from: token_owner });
            let errorThrown = false;
            try {
                let tx = await I_BlacklistTransferManager.deleteInvestorFromBlacklist(account_investor1, { from: account_investor2 });
            } catch(error) {
        ...
        it("Should fail in deleting the investor from the blacklist type", async() => {
            let errorThrown = false;
            try {
                let tx = await I_BlacklistTransferManager.deleteInvestorFromBlacklist(0x0, { from: token_owner });
            } catch(error) {
                console.log(`         tx revert -> Invalid investor address`.grey);
                errorThrown = true;
                ensureException(error);
        ...
        it("Should fail in deleting the investor from the blacklist type", async() => {
            let errorThrown = false;
            try {
                let tx = await I_BlacklistTransferManager.deleteInvestorFromBlacklist(account_investor2, { from: token_owner });
            } catch(error) {
    ...
    describe("Test cases for the factory", async() => {
        it("Should get the exact details of the factory", async() => {
            ...
            assert.equal(await I_BlacklistTransferManagerFactory.getType.call(),2);
