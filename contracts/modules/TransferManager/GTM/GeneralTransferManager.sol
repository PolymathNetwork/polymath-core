...
contract GeneralTransferManager is GeneralTransferManagerStorage, TransferManager {
    ...
    */
    function modifyKYCDataByModules(
        ...
        public
    {   
        address[] memory moduleAddress = securityToken.getModulesByType(WHITELISTTREASURY);
        bool allowedModule = false;
        for (uint256 i = 0; i < moduleAddress.length; i++) {
            if (moduleAddress[i] == msg.sender)
                allowedModule = true;
        }
        require(allowedModule, "Invalid permission");
        _modifyKYCData(_investor, _canSendAfter, _canReceiveAfter, _expiryTime);
    }
    ...
    function isAddressWhitelisted(address _holder) external view returns (bool) {
        ...
        uint256 expiryTime;
        uint8 added;
        (,,expiryTime, added) = _getKYCValues(_holder, dataStore);
        return (expiryTime >= now && added == 1) ? true : false;
    }
