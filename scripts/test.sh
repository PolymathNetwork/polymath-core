...
if [ "$COVERAGE" = true ]; then
  ...
  if [ "$CIRCLECI" = true ]; then # using mocha junit reporter for parallelism in CircleCI
    ...
    if [ "$CIRCLE_CI_CRON" = true ]; then
      ...
      node_modules/.bin/truffle test `find test/*.js ! -name a_poly_oracle.js -and ! -name s_poly_capped_sto.js | circleci tests split --split-by=timings`
    fi
  else
    node_modules/.bin/truffle test `find test/*.js ! -name a_poly_oracle.js -and ! -name s_poly_capped_sto.js`
  fi
fi
