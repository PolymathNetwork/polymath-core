...
const EtherDividendWithholdingCheckpointFactory = artifacts.require('./EtherDividendWithholdingCheckpointFactory.sol');
const EtherDividendWithholdingCheckpoint = artifacts.require('./EtherDividendWithholdingCheckpoint');
...
contract('EtherDividendWithholdingCheckpoint', accounts => {
    ...
    let P_EtherDividendWithholdingCheckpointFactory;
    let P_EtherDividendWithholdingCheckpoint;
    let I_EtherDividendWithholdingCheckpointFactory;
    let I_GeneralPermissionManager;
    let I_EtherDividendWithholdingCheckpoint;
    ...
    before(async() => {
        ...
        // STEP 4: Deploy the EtherDividendWithholdingCheckpoint
        I_EtherDividendWithholdingCheckpointFactory = await EtherDividendWithholdingCheckpointFactory.new(I_PolyToken.address, 0, 0, 0, {from:account_polymath});
        assert.notEqual(
            I_EtherDividendWithholdingCheckpointFactory.address.valueOf(),
            "0x0000000000000000000000000000000000000000",
            "EtherDividendWithholdingCheckpointFactory contract was not deployed"
        );
        ...
        // (C) : Register the EtherDividendWithholdingCheckpointFactory
        await I_ModuleRegistry.registerModule(I_EtherDividendWithholdingCheckpointFactory.address, { from: account_polymath });
        await I_ModuleRegistry.verifyModule(I_EtherDividendWithholdingCheckpointFactory.address, true, { from: account_polymath });

        // (C) : Register the Paid EtherDividendWithholdingCheckpointFactory
        await I_ModuleRegistry.registerModule(P_EtherDividendWithholdingCheckpointFactory.address, { from: account_polymath });
        await I_ModuleRegistry.verifyModule(P_EtherDividendWithholdingCheckpointFactory.address, true, { from: account_polymath });
    ...
    describe("Generate the SecurityToken", async() => {
        ...
        it("Should successfully attach the ERC20DividendCheckpoint with the security token", async () => {
            ...
            try {
                const tx = await I_SecurityToken.addModule(P_EtherDividendWithholdingCheckpointFactory.address, "", web3.utils.toWei("500", "ether"), 0, true, { from: token_owner });
            } catch(error) {
        ...
        it("Should successfully attach the EtherDividendWithholdingCheckpoint with the security token", async () => {
            let snapId = await takeSnapshot();
            await I_PolyToken.transfer(I_SecurityToken.address, web3.utils.toWei("500", "ether"), {from: token_owner});
            const tx = await I_SecurityToken.addModule(P_EtherDividendWithholdingCheckpointFactory.address, "", web3.utils.toWei("500", "ether"), 0, true, { from: token_owner });
            assert.equal(tx.logs[3].args._type.toNumber(), checkpointKey, "EtherDividendWithholdingCheckpoint doesn't get deployed");
            assert.equal(
                web3.utils.toAscii(tx.logs[3].args._name)
                .replace(/\u0000/g, ''),
                "EtherDividendWithholdingCheckpoint",
                "EtherDividendWithholdingCheckpoint module was not added"
            );
            P_EtherDividendWithholdingCheckpoint = EtherDividendWithholdingCheckpoint.at(tx.logs[3].args._module);
            await revertToSnapshot(snapId);
        });

        it("Should successfully attach the EtherDividendWithholdingCheckpoint with the security token", async () => {
            const tx = await I_SecurityToken.addModule(I_EtherDividendWithholdingCheckpointFactory.address, "", 0, 0, true, { from: token_owner });
            assert.equal(tx.logs[2].args._type.toNumber(), checkpointKey, "EtherDividendWithholdingCheckpoint doesn't get deployed");
            assert.equal(
            ...
            I_EtherDividendWithholdingCheckpoint = EtherDividendWithholdingCheckpoint.at(tx.logs[2].args._module);
        });
    });
    ...
    describe("Check Dividend payouts", async() => {
        ...
        it("Should fail in creating the dividend", async() => {
            ...
            try {
                let tx = await I_EtherDividendWithholdingCheckpoint.createDividend(maturity, expiry, {from: token_owner});
            } catch(error) {
        ...
        it("Should fail in creating the dividend", async() => {
            ...
            let expiry = latestTime() - duration.days(10);
            try {
                let tx = await I_EtherDividendWithholdingCheckpoint.createDividend(maturity, expiry, {from: token_owner, value: web3.utils.toWei('1.5', 'ether')});
            } catch(error) {
        ...
        it("Issuer pushes dividends iterating over account holders - dividends proportional to checkpoint", async() => {
            let errorThrown = false;
            try {
                await I_EtherDividendWithholdingCheckpoint.pushDividendPayment(0, 0, 10, {from: token_owner});
            } catch(error) {
        ...
        it("Issuer pushes dividends iterating over account holders - dividends proportional to checkpoint", async() => {
            ...
            try {
                await I_EtherDividendWithholdingCheckpoint.pushDividendPayment(0, 0, 10, {from: account_temp});
            } catch(error) {
        ...
        it("Issuer pushes dividends iterating over account holders - dividends proportional to checkpoint", async() => {
            let errorThrown = false;
            try {
                await I_EtherDividendWithholdingCheckpoint.pushDividendPayment(2, 0, 10, {from: token_owner});
            } catch(error) {
        ...
        it("Issuer pushes dividends iterating over account holders - dividends proportional to checkpoint", async() => {
            ...
            //Check fully claimed
            assert.equal((await I_EtherDividendWithholdingCheckpoint.dividends(0))[5].toNumber(), web3.utils.toWei('1.5', 'ether'));
        });
        ...
        it("Issuer pushes dividends iterating over account holders - dividends proportional to checkpoint", async() => {
         ...
         it("Issuer pushes dividends iterating over account holders - dividends proportional to checkpoint", async() => {
            let errorThrown = false;
            let tx = await I_EtherDividendWithholdingCheckpoint.reclaimDividend(1, {from: token_owner, gas: 500000});
            assert.equal((tx.logs[0].args._claimedAmount).toNumber(), web3.utils.toWei("1.5", "ether"));
            try {
                await I_EtherDividendWithholdingCheckpoint.reclaimDividend(1, {from: token_owner, gas: 500000});
            } catch(error) {
        ...
        it("Create another new dividend", async() => {
            let maturity = latestTime();
            let expiry = latestTime() + duration.days(10);
            let tx = await I_EtherDividendWithholdingCheckpoint.createDividend(maturity, expiry, {from: token_owner, value: web3.utils.toWei('11', 'ether')});
        ...
        it("should investor 3 claims dividend", async() => {
            ...
            try {
                await I_EtherDividendWithholdingCheckpoint.pullDividendPayment(5, {from: account_investor3, gasPrice: 0});
            } catch(error) {
        ...
        it("Should investor 3 claims dividend", async() => {
            ...
            await I_EtherDividendWithholdingCheckpoint.pullDividendPayment(2, {from: account_investor3, gasPrice: 0});
        ...
        it("Issuer pushes remainder", async() => {
            ...
            //Check fully claimed
            assert.equal((await I_EtherDividendWithholdingCheckpoint.dividends(2))[5].toNumber(), web3.utils.toWei('11', 'ether'));
        });
        ...
        it("Create another new dividend with explicit", async() => {
            ...
            try {
                tx = await I_EtherDividendWithholdingCheckpoint.createDividendWithCheckpoint(maturity, expiry, 4, {from: token_owner, value: 0});
            } catch(error) {
        ...
        it("Create another new dividend with explicit", async() => {
            ...
            let expiry = latestTime() - duration.days(10);
            try {
                tx = await I_EtherDividendWithholdingCheckpoint.createDividendWithCheckpoint(maturity, expiry, 4, {from: token_owner, value: web3.utils.toWei('11', 'ether')});
            } catch(error) {
                console.log(`       tx -> failed because maturity > expiry`.grey);
                ensureException(error);
                errorThrown = true;
        ...
        it("Create another new dividend with explicit", async() => {
            ...
            let expiry = latestTime() + duration.days(2);
            try {
                tx = await I_EtherDividendWithholdingCheckpoint.createDividendWithCheckpoint(maturity, expiry, 5, {from: token_owner, value: web3.utils.toWei('11', 'ether')});
            } catch(error) {
        ...
        it("Investor 2 claims dividend, issuer pushes investor 1", async() => {
            ...
            let investor3Balance = BigNumber(await I_PolyToken.balanceOf(account_investor3));
            try {
                await I_EtherDividendWithholdingCheckpoint.pushDividendPaymentToAddresses(4, [account_investor2, account_investor1],{from: account_investor2, gasPrice: 0});
            } catch(error) {
        ...
        it("Investor 2 claims dividend, issuer pushes investor 1", async() => {
            ...
            let investor3Balance = BigNumber(await I_PolyToken.balanceOf(account_investor3));
            try {
                await I_EtherDividendWithholdingCheckpoint.pushDividendPaymentToAddresses(6, [account_investor2, account_investor1],{from: token_owner, gasPrice: 0});
            } catch(error) {
                console.log(`       tx -> failed because dividend index is not valid`.grey);
                ensureException(error);
                errorThrown = true;
        ...
        it("should calculate dividend before the push dividend payment", async() => {
            let dividendAmount1 = await I_EtherDividendWithholdingCheckpoint.calculateDividend.call(3, account_investor1);
            let dividendAmount2 = await I_EtherDividendWithholdingCheckpoint.calculateDividend.call(3, account_investor2);
            let dividendAmount3 = await I_EtherDividendWithholdingCheckpoint.calculateDividend.call(3, account_investor3);
            let dividendAmount_temp = await I_EtherDividendWithholdingCheckpoint.calculateDividend.call(3, account_temp);
        ...
        it("Investor 2 claims dividend", async() => {
            ...
            await I_EtherDividendWithholdingCheckpoint.pullDividendPayment(3, {from: account_investor2, gasPrice: 0});
            let investor1BalanceAfter1 = BigNumber(await web3.eth.getBalance(account_investor1));
            let investor2BalanceAfter1 = BigNumber(await web3.eth.getBalance(account_investor2));
            let investor3BalanceAfter1 = BigNumber(await web3.eth.getBalance(account_investor3));
        ...
        it("Should issuer pushes investor 1 and temp investor", async() => {
            ...
            let investor3BalanceAfter1 = BigNumber(await web3.eth.getBalance(account_investor3));
            let tempBalanceAfter1 = BigNumber(await web3.eth.getBalance(account_temp));
            await I_EtherDividendWithholdingCheckpoint.pushDividendPaymentToAddresses(3, [account_investor1, account_temp], {from: token_owner});
            let investor1BalanceAfter2 = BigNumber(await web3.eth.getBalance(account_investor1));
            let investor2BalanceAfter2 = BigNumber(await web3.eth.getBalance(account_investor2));
            let investor3BalanceAfter2 = BigNumber(await web3.eth.getBalance(account_investor3));
            ...
            //Check fully claimed
            assert.equal((await I_EtherDividendWithholdingCheckpoint.dividends(3))[5].toNumber(), web3.utils.toWei('4', 'ether'));
        });
        ...
        it("Issuer unable to reclaim dividend (expiry not passed)", async() => {
            let errorThrown = false;
            try {
                await I_EtherDividendWithholdingCheckpoint.reclaimDividend(3, {from: token_owner});
            } catch(error) {
        ...
        it("Issuer is able to reclaim dividend after expiry", async() => {
            ...
            try {
                await I_EtherDividendWithholdingCheckpoint.reclaimDividend(8, {from: token_owner, gasPrice: 0});
            } catch(error) {
                console.log(`       tx -> failed because dividend index is not valid`.grey);
                ensureException(error);
                errorThrown = true;
         ...
         it("Issuer is able to reclaim dividend after expiry", async() => {
            ...
            await I_EtherDividendWithholdingCheckpoint.reclaimDividend(3, {from: token_owner, gasPrice: 0});
        ...
        it("Investor 3 unable to pull dividend after expiry", async() => {
            let errorThrown = false;
            try {
                await I_EtherDividendWithholdingCheckpoint.pullDividendPayment(3, {from: account_investor3, gasPrice: 0});
            } catch(error) {
        ...
        it("Should give the right dividend index", async() => {
            let index = await I_EtherDividendWithholdingCheckpoint.getDividendIndex.call(3);
            assert.equal(index[0], 2);
        });

        it("Should give the right dividend index", async() => {
            let index = await I_EtherDividendWithholdingCheckpoint.getDividendIndex.call(8);
            assert.equal(index.length, 0);
        });

        it("Get the init data", async() => {
            let tx = await I_EtherDividendWithholdingCheckpoint.getInitFunction.call();
            assert.equal(web3.utils.toAscii(tx).replace(/\u0000/g, ''),0);
        });

        it("Should get the listed permissions", async() => {
            let tx = await I_EtherDividendWithholdingCheckpoint.getPermissions.call();
            assert.equal(tx.length,0);
        });

        describe("Test cases for the EtherDividendWithholdingCheckpointFactory", async() => {
            it("should get the exact details of the factory", async() => {
                assert.equal((await I_EtherDividendWithholdingCheckpointFactory.setupCost.call()).toNumber(), 0);
                assert.equal(await I_EtherDividendWithholdingCheckpointFactory.getType.call(), 4);
                assert.equal(web3.utils.toAscii(await I_EtherDividendWithholdingCheckpointFactory.getName.call())
                            .replace(/\u0000/g, ''),
                            "EtherDividendWithholdingCheckpoint",
                            "Wrong Module added");
                assert.equal(await I_EtherDividendWithholdingCheckpointFactory.getDescription.call(),
                ...
                assert.equal(await I_EtherDividendWithholdingCheckpointFactory.getTitle.call(),
                            ...
                            "Wrong Module added");
                assert.equal(await I_EtherDividendWithholdingCheckpointFactory.getInstructions.call(),
                            ...
                            "Wrong Module added");
                let tags = await I_EtherDividendWithholdingCheckpointFactory.getTags.call();
