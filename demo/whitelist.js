...
function readFile() {
  ...
  var csvStream = csv()
    .on("data", function (data) {
      ...
      let restrictionToBuy = (typeof JSON.parse(data[4].toLowerCase())) == "boolean" ? JSON.parse(data[4].toLowerCase()) : "not-valid";

      if (isAddress && sellValid && buyValid && kycExpiryDate && (restrictionToBuy != "not-valid") ) {
        ...
        userArray.push(restrictionToBuy)
...
async function setInvestors() {
  ...
  for (let i = 0; i < distribData.length; i++) {
    try {
      ...
      for (let j = 0; j < distribData[i].length; j++) {
        ...
        resctrictionArray.push(distribData[i][j][4])
      }
      ...
      let r = await generalTransferManager.methods.modifyWhitelistMulti(investorArray, fromTimesArray, toTimesArray, expiryTimeArray, resctrictionArray).send({ from: Issuer, gas: 5000000, gasPrice: DEFAULT_GAS_PRICE })
  ...
  for (var i = 0; i < event_data.length; i++) {
    ...
    let restrictedToBuy_Event = event_data[i].returnValues._restrictedToBuy
    ...
    combineArray.push(restrictedToBuy_Event);
    ...
    if (investorObjectLookup.hasOwnProperty(investorAddress_Event)) {
      ...
      if (investorObjectLookup[investorAddress_Event].recordedBlockNumber < blockNumber) {
        investorObjectLookup[investorAddress_Event] = { fromTime: fromTime_Event, toTime: toTime_Event, expiryTime: expiryTime_Event, restrictedToBuy: restrictedToBuy_Event, recordedBlockNumber: blockNumber };
        updatedInvestors += 1;
