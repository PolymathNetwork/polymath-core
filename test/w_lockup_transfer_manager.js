...
contract('LockUpTransferManager', accounts => {
    ...
    describe("Generate the SecurityToken", async() => {
        ...
        it("Should generate the new security token with the same symbol as registered above", async () => {
            ...
            let tx = await I_STRProxied.generateSecurityToken(name, symbol, tokenDetails, false, token_owner, { from: token_owner });
        ...
        it("Should generate the new security token with the same symbol as registered above", async () => {
            await I_PolyToken.approve(I_STRProxied.address, initRegFee, { from: token_owner });
            let _blockNo = latestBlock();
            let tx = await I_STRProxied.generateSecurityToken(name2, symbol2, tokenDetails, true, token_owner, { from: token_owner });
