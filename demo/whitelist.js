...
async function setInvestors() {
  ...
  for (let i = 0; i < distribData.length; i++) {
    try {
      ...
      console.log(chalk.red(`modifyWhitelistMulti: ` + await generalTransferManager.methods.modifyWhitelistMulti(investorArray, fromTimesArray, toTimesArray, expiryTimeArray, canBuyFromSTOArray).estimateGas({from: Issuer})));
