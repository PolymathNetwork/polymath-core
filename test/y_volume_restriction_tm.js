...
contract('VolumeRestrictionTransferManager', accounts => {
    ...
    before(async() => {
        currentTime = new BN(await currentTime);
        fromTime = currentTime;
        toTime = currentTime;
        expiryTime = toTime.add(duration.days(15));
        // Accounts setup
    ...
    describe("Attach the VRTM", async() => {
        ...
        it("Transfer some tokens to different account", async() => {
            ...
            await I_GeneralTransferManager.modifyWhitelistMulti(
                    ...
                    [currentTime, currentTime, currentTime],
                    [currentTime.add(duration.days(60)), currentTime.add(duration.days(60)), currentTime,add(duration.days(60))],
                    [true, true, true],
                    {
                        from: token_owner
                    }
    ...
    describe("Test for the addIndividualRestriction", async() => {
        ...
        it("Should add the restriction -- failed because of bad owner", async() => {
            await catchRevert(
                I_VolumeRestrictionTM.addIndividualRestriction(
                    ...
                    currentTime.add(duration.seconds(2)),
                    3,
                    currentTime.add(duration.days(10)),
                    0,
                    {
        ...
        it("Should add the restriction -- failed because of bad parameters i.e invalid dates", async() => {
            await catchRevert(
                I_VolumeRestrictionTM.addIndividualRestriction(
                    account_investor1,
                    new BN(web3.utils.toWei("10")),
                    currentTime.sub(duration.seconds(5)),
                    3,
                    currentTime.add(duration.days(10)),
                    0,
                    {
        ...
        it("Should add the restriction -- failed because of bad parameters i.e invalid rolling period", async() => {
            await catchRevert(
                I_VolumeRestrictionTM.addIndividualRestriction(
                    ...
                    currentTime.add(duration.days(2)),
                    3,
                    currentTime.add(duration.days(3)),
                    0,
                    {
                        from: token_owner
                    }
        ...
        it("Should add the restriction succesfully", async() => {
            let tx = await I_VolumeRestrictionTM.addIndividualRestriction(
                    ...
                    currentTime.add(duration.seconds(2)),
                    3,
                    currentTime.add(duration.days(5)),
                    0,
                    {
                        from: token_owner
                    }
        ...
        it("Should add the restriction for multiple investor -- failed because of bad owner", async() => {
            await catchRevert(
                I_VolumeRestrictionTM.addIndividualRestrictionMulti(
                    ...
                    [currentTime.add(duration.seconds(2)), currentTime.add(duration.seconds(2)), currentTime.add(duration.seconds(2))],
                    [3,4,5],
                    [currentTime.add(duration.days(5)), currentTime.add(duration.days(6)), currentTime.add(duration.days(7))],
                    [0,0,0],
                    {
                        from: account_polymath
                    }
        ...
        it("Should add the restriction for multiple investor -- failed because of bad parameters i.e length mismatch", async() => {
            await catchRevert(
                I_VolumeRestrictionTM.addIndividualRestrictionMulti(
                    [account_investor2, account_delegate3, account_investor4],
                    [new BN(web3.utils.toWei("12")), new BN(web3.utils.toWei("10")), new BN(web3.utils.toWei("15"))],
                    [currentTime.add(duration.seconds(2)), currentTime.add(duration.seconds(2))],
                    [3,4,5],
                    [currentTime.add(duration.days(5)), currentTime.add(duration.days(6)), currentTime.add(duration.days(7))],
                    [0,0,0],
                    {
        ...
        it("Should add the restriction for multiple investor successfully", async() => {
            ...
            assert.equal((await I_VolumeRestrictionTM.individualRestriction.call(account_investor2))[2].toString(), 3);
            assert.equal((await I_VolumeRestrictionTM.individualRestriction.call(account_delegate3))[2].toString(), 4);
            assert.equal((await I_VolumeRestrictionTM.individualRestriction.call(account_investor4))[2].toString(), 5);
        ...
        it("Should successfully remove the restriction", async() => {
            ...
            assert.equal((await I_VolumeRestrictionTM.individualRestriction.call(account_investor2))[3].toString(), 0);
            let data = await I_VolumeRestrictionTM.getRestrictedData.call();
            await printRestrictedData(data);
        ...
        it("Should add the restriction succesfully after the expiry of previous one for investor 1", async() => {
            ...
            console.log(
                ...
                ${await I_VolumeRestrictionTM.addIndividualRestriction.estimateGas(
                    ...
                    currentTime.add(duration.seconds(2)),
                    3,
                    currentTime.add(duration.days(6)),
                    0,
                    {
                        from: token_owner
                    }
        ...
        it("Should succesfully transact the tokens by investor 1 just after the startTime", async() => {
            ...
            let result = await I_VolumeRestrictionTM.verifyTransfer.call(account_investor1, account_investor3, new BN(web3.utils.toWei('.3')), "0x0", false);
            assert.equal(result.toString(), 1);
            // Perform the transaction
            console.log(`
                Gas estimation (Individual): ${await I_SecurityToken.transfer.estimateGas(account_investor3, new BN(web3.utils.toWei('.3')), {from: account_investor1})}`
            );
            ...
            assert.equal(
                data[0].toString(),
                (await I_VolumeRestrictionTM.individualRestriction.call(account_investor1))[1].toString()
            );
            assert.equal(web3.utils.fromWei((data[1].toString()).toString()), 0.3);
            tempArray.push(0.3);
        });
        ...
        it("Should fail to add the individual daily restriction -- Bad msg.sender", async() => {
            await catchRevert(
                I_VolumeRestrictionTM.addIndividualDailyRestriction(
                    ...
                    currentTime.add(duration.days(4)),
                    0,
                    {
        ...
        it("Should add the individual daily restriction for investor 3", async() => {
            ...
            console.log(`
                *** Individual Daily restriction data ***
                Allowed Tokens:          ${dataRestriction[0].dividedBy(new BN(10).pow(18)).toString()}
        ...
        it("Should transfer the tokens within the individual daily restriction limits", async() => {
            ...
            let startTime = (await I_VolumeRestrictionTM.individualDailyRestriction.call(account_investor3))[1].toString();
            console.log(`
            ...
            .dividedBy(new BN(10).pow(18)).toString(), 6);
        });
        ...
        it("Should try to send after the one day completion", async() => {
            ...
            assert.equal(data[3].toString(), startTime + duration.days(1));
            assert.equal((await I_VolumeRestrictionTM.getTotalTradedByUser.call(account_investor3, data[3]))
            .dividedBy(new BN(10).pow(18)).toString(), 2);
        });
        ...
        it("Should add the daily restriction on the investor 1", async() => {
            ...
            assert.equal((tx.logs[0].args._allowedTokens).dividedBy(new BN(10).pow(16)).toString(), 5);
            let data = await I_VolumeRestrictionTM.getRestrictedData.call();
            await printRestrictedData(data);
            assert.equal(data[0].length, 3);
            ...
            console.log(`
                *** Individual Daily restriction data ***
                Allowed Tokens:          ${dataRestriction[0].dividedBy(new BN(10).pow(16)).toString()} % of TotalSupply
                StartTime :              ${dataRestriction[1].toString()}
                Rolling Period in days : ${dataRestriction[2].toString()}
                EndTime :                ${dataRestriction[3].toString()}
                Type of Restriction:     ${dataRestriction[4].toString()}
        ...
        it("Should transfer tokens on the 2nd day by investor1 (Individual + Individual daily)", async() => {
            let startTime = (await I_VolumeRestrictionTM.individualRestriction.call(account_investor1))[1].toString();
            let rollingPeriod = (await I_VolumeRestrictionTM.individualRestriction.call(account_investor1))[2].toString();
            ...
            let bal1 = await I_SecurityToken.balanceOf.call(account_investor1);
            // Verifying the balances
            assert.equal(web3.utils.fromWei((bal1.toString()).toString()), 32.7);
            tempArray.push(2);
            ...
            let amt = (await I_VolumeRestrictionTM.getTotalTradedByUser.call(account_investor1, data[0].toString()))
            .dividedBy(new BN(10).pow(18)).toString();

            // Verify the storage changes
            assert.equal(data[0].toString(), startTime + duration.days(data[2].toString()));
            assert.equal(data[1].dividedBy(new BN(10).pow(18)).toString(), await calculateSum(rollingPeriod, tempArray));
            assert.equal(data[2].toString(), 1);
            assert.equal(data[3].toString(),
             (await I_VolumeRestrictionTM.individualDailyRestriction.call(account_investor1))[1].toString());
            assert.equal(amt, 2);
        });
        ...
        it("Should add the individual restriction to investor 3", async() => {
            let tx = await I_VolumeRestrictionTM.addIndividualRestriction(
                ...
                currentTime.add(duration.seconds(2)),
                6,
                currentTime.add(duration.days(15)),
                1,
                {
                    from: token_owner
                }
        ...
        it("Should transfer the token by the investor 3 with in the (Individual + Individual daily limit)", async() => {
            ...
            let startTime = (await I_VolumeRestrictionTM.individualRestriction.call(account_investor3))[1].toString();
            let rollingPeriod = (await I_VolumeRestrictionTM.individualRestriction.call(account_investor3))[2].toString();
            let startTimeDaily = (await I_VolumeRestrictionTM.individualDailyRestriction.call(account_investor3))[1].toString();
            console.log(`
            ...
            // Verifying the balances
            assert.equal(web3.utils.fromWei(((bal1.minus(bal2)).toString()).toString()), 4);
            ...
            // Verify the storage changes
            assert.equal(data[0].toString(), startTime + duration.days(data[2].toString()));
            assert.equal(data[1].dividedBy(new BN(10).pow(18)).toString(), 4);
            assert.equal(data[2].toString(), 0);
            assert.equal(data[3].toString(), startTimeDaily + duration.days(1));
            assert.equal(amt, 4);
        ...
        it("Should remove the daily individual limit and transfer more tokens on a same day", async() => {
            ...
            // Verify the storage changes
            assert.equal(data[0].toString(), startTime + duration.days(data[2].toString()));
            assert.equal(data[1].dividedBy(new BN(10).pow(18)).toString(), 8);
            assert.equal(data[2].toString(), 0);
        ...
        it("Should add the new Individual daily restriction and transact the tokens", async() => {
            ...
            // Verify the storage changes
            assert.equal(data[0].toString(), startTime + duration.days(data[2].toString()));
            assert.equal(data[1].dividedBy(new BN(10).pow(18)).toString(), await calculateSum(rollingPeriod, tempArray3));
            assert.equal(data[2].toString(), 1);
    ...
    describe("Test cases for the Default restrictions", async() => {
        ...
        it("Should add the investor 4 in the whitelist", async() => {
            await I_GeneralTransferManager.modifyWhitelist(
                ...
                currentTime,
                currentTime,
                currentTime.add(duration.days(30)),
                true,
                {
                    from: token_owner
                }
        ...
        it("Should transfer the token by investor 4", async() => {
            let startTimedaily = (await I_VolumeRestrictionTM.defaultDailyRestriction.call())[1].toString();
            //sell tokens upto the limit
        ...
        it("Should transfer tokens on by investor 3 (comes under the Default restriction)", async() => {
            ...
            tempArray3.length = 0;
            let startTime = (await I_VolumeRestrictionTM.defaultRestriction.call())[1].toString();
            let startTimedaily = (await I_VolumeRestrictionTM.defaultDailyRestriction.call())[1].toString();
            let rollingPeriod = (await I_VolumeRestrictionTM.defaultRestriction.call())[2].toString();
            //sell tokens upto the limit
        ...
        it("Should add the daily default restriction again", async() => {
            ...
            console.log(`
                *** Add Individual Daily restriction data ***
                Allowed Tokens:          ${dataRestriction[0].dividedBy(new BN(10).pow(16)).toString()}
                StartTime :              ${dataRestriction[1].toString()}
                Rolling Period in days : ${dataRestriction[2].toString()}
                EndTime :                ${dataRestriction[3].toString()}
                Type of Restriction:     ${dataRestriction[4].toString()}
    ...
    describe("Test for modify functions", async() => {
        ...
        it("Should add the individual restriction for multiple investor", async() => {
            await I_VolumeRestrictionTM.addIndividualRestrictionMulti(
                ...
                [currentTime.add(duration.days(1)), currentTime.add(duration.days(2))],
                [15, 20],
                [currentTime.add(duration.days(40)), currentTime.add(duration.days(60))],
                [0,1],
                {
                    from: token_owner
                }
            ...
            assert.equal(indi1[0].dividedBy(new BN(10).pow(18)), 15);
            assert.equal(indi2[0].dividedBy(new BN(10).pow(16)), 12.78);
        ...
        it("Should modify the details before the starttime passed", async() => {
            ...
            assert.equal(indi2[0].dividedBy(new BN(10).pow(18)), 15);
            assert.equal(indi1[0].dividedBy(new BN(10).pow(16)), 12.78);
