...
contract SecurityTokenRegistry is ISecurityTokenRegistry, Util, IRegistry {
    ...
    public
    {
        POLY_Address = _polyAddress;
        MR_Address = _moduleRegistry;
        TR_Address = _tickerRegistry;
    ...
    function generateSecurityToken(string _name, string _symbol, string _tokenDetails, bool _divisible) public whenNotPaused {
        ...
        require(ITickerRegistry(TR_Address).checkValidity(_symbol, msg.sender, _name), "Trying to use non-valid symbol");
        if(registrationFee > 0)
            require(ERC20(POLY_Address).transferFrom(msg.sender, this, registrationFee), "Failed transferFrom because of sufficent Allowance is not provided");
    ...
    function addCustomSecurityToken(string _name, string _symbol, address _owner, address _securityToken, string _tokenDetails, bytes32 _swarmHash) public onlyOwner whenNotPaused {
        ...
        require(!(ITickerRegistry(TR_Address).isReserved(_symbol, _owner, _name, _swarmHash)), "Trying to use non-valid symbol");
