...
contract('SecurityToken', accounts => {
  ...
  describe('Generate the SecurityToken', async () => {
    ...
    it('Should mintMulti', async () => {
      await catchRevert(
        I_SecurityToken.mintMulti([account_affiliate1, account_affiliate2], [100 * Math.pow(10, 18)], { from: token_owner, gas: 500000 })
      );
    });
  ...
  describe('General Transfer manager Related test cases', async () => {
    ...
    it('Should force burn the tokens - value too high', async () => {
      ...
      await catchRevert(
        I_SecurityToken.forceBurn(account_temp, currentBalance + web3.utils.toWei('500', 'ether'), '', { from: account_controller })
      );
    });
  ...
  describe('Force Transfer', async () => {
    ...
    it('Should successfully forceTransfer', async () => {
      ...
      let tx = await I_SecurityToken.forceTransfer(account_investor1, account_investor2, web3.utils.toWei('10', 'ether'), 'reason', {
        from: account_controller
      });
