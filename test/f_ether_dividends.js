...
contract('EtherDividendCheckpoint', accounts => {
  ...
  before(async () => {
    ...
    P_EtherDividendCheckpointFactory = await EtherDividendCheckpointFactory.new(
      I_PolyToken.address,
  ...
  describe('Generate the SecurityToken', async () => {
    ...
    it('Should successfully attach the ERC20DividendCheckpoint with the security token', async () => {
      ...
      await catchRevert(
        I_SecurityToken.addModule(P_EtherDividendCheckpointFactory.address, '', web3.utils.toWei('500', 'ether'), 0, { from: token_owner })
      );
    });
  ...
  describe('Check Dividend payouts', async () => {
    ...
    it('Should fail in creating the dividend', async () => {
      ...
      let expiry = latestTime() + duration.days(10);
      await catchRevert(
        I_EtherDividendCheckpoint.createDividend(maturity, expiry, '', { from: token_owner, value: web3.utils.toWei('1.5', 'ether') })
      );
    });
